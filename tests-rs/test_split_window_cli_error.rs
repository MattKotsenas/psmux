...
fn missing_option_values_fail_before_cli_dispatch() {
    for option in ["-c", "-e", "-F", "-l", "-p", "-T", "-t"] {
        ...
        let profile = std::env::temp_dir().join(format!(
            "psmux_split_window_cli_error_{}_{}_{}",
            std::process::id(),
...
#[test]
fn process_cli_serializes_split_window_from_the_typed_parser() {
    let listener = TcpListener::bind("127.0.0.1:0").expect("bind mock control port");
    let port = listener.local_addr().unwrap().port();
    let profile = std::env::temp_dir().join(format!(
        "psmux_split_window_cli_wire_{}_{port}",
        std::process::id()
    ));
    let psmux_dir = profile.join(".psmux");
    fs::create_dir_all(&psmux_dir).unwrap();
    ...
    let server = thread::spawn(move || {
            ...
            match listener.accept() {
                Ok((mut stream, _)) => {
                    ...
                    loop {
                        ...
                        if line.starts_with("split-window ") {
                            stream.write_all(b"created\n").unwrap();
                            stream.flush().unwrap();
                            return request;
                        }
                    }
        ...
        panic!("split-window request was not received");
    });
    ...
    let command = request
        ...
        .unwrap_or_else(|| panic!("split-window request missing from {request:?}"));
    assert_eq!(
        command,
        "split-window -v -P -Z -F \"secondfmt\" -c \"second\" -T \"second-title\" -l \"10\" -e \"A=1\" -e \"B=two\" -- \"tool\" \"\" \"wide arg\""
    );
}
